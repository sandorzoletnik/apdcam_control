...
namespace apdcam10g
{
    ...
    // The template parameter is a class that is associated with a given channel. It is substituted
    // with two different types:
    // In apdcam10g::fake_camera it only stores channel info (chnanel number, etc)
    // In apdcam10g::daq it contains a ring_buffer as well.
    ...
    public:
        ...
        //html-target daq_settings::n_adc
        // Return the number of active ADC boards (those which have enabled channels)
        unsigned int n_adc() const { std::shared_lock lck(board_enabled_channels_); return board_enabled_channels_.size(); }

        //html-target daq_settings::n_channels
        // Return the number of physically present channels
        unsigned int n_channels() const { std::shared_lock lck(all_channels_); return all_channels_.size(); }

        //html-target daq_settings::n_enabled_channels
        // Return the number of all enabled channels
        unsigned int n_enabled_channels() { return all_enabled_channels_.size(); }
        ...
        // For the set network interface, get the MTU from the system settings, calculate octet, etc
        daq_settings &get_net_parameters();

        // Set the channel masks. A vector of vector of bools, first index is the ADC board number (0-based), and
        // the second index is the channel number (0..31) of that board
...
//            for(auto p : all_enabled_channels_buffers_) delete p;
        ...
        // Returns the ring_buffer and other data associated with the given channel
...
//                for(auto e : extractors_) if(e) e->debug(d);
        ...
        // Set a sampling number for all diskdump (exiting and future) processors. That is, only shot numbers being an integer multiple
...
extern "C"
{
    ...
    //html-target n_adc
    // Get the number of active ADC boards (those which have enabled channels)
    // A wrapper around daq_settings::n_adc()
    unsigned int n_adc();

    //html-target n_channels
    // Return the number of physically present channels (Number of present ADC boards * 32)
    // A wrapper around daq_settings::n_channels
    unsigned int n_channels();

    //html-target n_enabled_channels
    // Return the number of all enabled channels
    // A wrapper around daq_ettings::n_enabled_channels
    unsigned int n_enabled_channels();

    //html-target start
    // Start the DAQ, and optinally wait for it to finish, blocking execution
    void start(bool wait=false);

    //html-target stop
    // Signal the DAQ process to stop, and optinally wait until it does so
    void stop(bool wait=true);

    // Kill all threads, and the DAQ process
    // both 'kill' and 'abort' would conflict with existing standard C library functions
    void kill_all();  
    void version(apdcam10g::version v);
    void dual_sata(bool d);

    //html-target channel_masks
    // C (low-level) wrapper around daq::channel_masks(vector<vector<bool>>)
    // 'm' is a pointer of pointer to bool (i.e. a 2-level nested C array)
    // containing 'true' for enabled channels. The top-most size of the
    // array should be 'n_adc_boards', the 1st level size should be 32 (number
    // of channels per board)
    // 'n_adc_boards' is the number of present ADC boards, 
    void channel_masks(bool **m, int n_adc_boards);

    // Set the resolution (number of bits) individually for each ADC board.
    // First argument 'r' is a pointer to the array of values, which should have
    // 'n_adc_boards' elements
    ...
    // Query the status of the threads
    ...
    //html-target diskdump_sampling
    // Set the sampling interval for diskdump tasks (writing channel data to disk) for existing and
    // future processor_diskdump tasks. A value 's' means everh sth value is written to disk
    // A wrapper around daq::diskdump_sampling
    void diskdump_sampling(unsigned int s);
...
//    void status(unsigned int *n_active_network_threads, unsigned int *n_active_extractor_threads, unsigned int *n_active_processor_thread);
    ...
    // For the given channel, return the size of the ring buffer associated with this channel (in the variable 'buffersize'),
    // and the underlying raw C memory array (in the variable 'buffer')
    ...
    // To be called from a python analysis task: set/communicate the earliest data counter to the C++ DAQ backend
    ...
    //html-target python_analysis_stop
    // Query the 'stop' flag by which the C++ backend communicates to the python analysis thread
    // to stop
    // A wrapper around daq::python_analysis_stop
    bool python_analysis_stop();
