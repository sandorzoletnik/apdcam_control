...
#        self.daq = ctypes.CDLL("c++/libapdcam10g.so")
#        if self.daq is None:
#            print("Could not load shared library libapdcam10g.so")
#            return "Could not load shared library libapdcam10g.so"
...
#                d[5 - i] = (sampleNumber // 2 ** (i * 8)) % 256
    ...
    def measure(self,
        ...
        Parameters
        ^^^^^^^^^^
        processorTasks : a list of processor tasks.
            Each element can be either an instance of a python processor (these are defined in Processor.py), or
            a string (name of a C++ processor task) referring to a C++ processor task, for which the daq.h header
            file should define a function named 'add_processor_XXXX' where XXXX is the string (name of the processor
            task). For example, the string can be "diskdump" because daq.h defines the extern "C" void add_processor_diskdump() function
        streamMode : string
        ...
        channelMasks : list of list of booleans. Length must be the same as number of ADC boards, and the
            length of each of its members must be the number of channels per board. Optional.
        ...
        resolutionBits : either a list of integers, or a single integer
            The number of bits in sampling. If it is a list, the length of the list must be equal to the number
            of ADC boards, and each element defines the resolution for the given ADC board. If a single number,
            it refers to the resolution of all ADC boards
        waitForResult : int, optional
             <=0 : Do not wait for the DAQ to finish
             > 0 : Wait this much seconds, and then request the DAQ to stop
...
#        DAQ().get_net_parameters()
        ...
        for task in processorTasks:
            ...
            if type(task) is tuple:
                if type(task[0]) is not str:
                    return "The first element of a tuple must be a string"
                f = getattr(DAQ(),"add_processor_"+task[0])
                if f is None:
                    return "The shared DAQ library 'libapdcam10g.so' does not have a function named '"+task+"'","",None
    ...
    def measurementStatus(self):
        ...
        err = ""
        warning = ""
        [n_packets,n_shots] = DAQ().statistics()
        run_stat = "Number of packets: " + str(n_packets) + ", number of shots: " + str(n_shots)
        [n_network_threads,n_extractor_threads,n_processor_thread] = DAQ().status()
        if n_network_threads>0 or n_extractor_threads>0 or n_processor_threads>0:
            run_stat += " Threads: " + str(n_network_threads) + "/" + str(n_extractor_threads) + "/" + str(n_processor_thread)
        else:
            run_stat = "Finished"
